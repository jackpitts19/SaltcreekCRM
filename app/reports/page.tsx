...
export default function ReportsPage() {
  ...
  function exportCSV() {
    ...
    rows.push(["Total Contacts", data.totalContacts]);
    rows.push(["Total Companies", data.totalCompanies]);
    rows.push(["Total Deals", data.totalDeals]);
    rows.push(["Pipeline Value ($M)", data.pipelineValue]);
    rows.push(["Closed Won ($M)", data.closedWonValue]);
    rows.push(["Total Emails", data.totalEmails]);
    rows.push(["Total Calls", data.totalCalls]);
    rows.push([]);
    rows.push(["PIPELINE BY STAGE"]);
    rows.push(["Stage", "Count", "Value ($M)"]);
    data.dealsByStage.forEach(s => rows.push([getDealStage(s.stage).label, s.count, s.value]));
    rows.push([]);
    rows.push(["RECENT WINS"]);
    rows.push(["Deal Name", "Value ($M)"]);
    data.recentWins.forEach(d => rows.push([d.name, d.value ?? ""]));
    rows.push([]);
    rows.push(["MOST ACTIVE CONTACTS"]);
    rows.push(["Name", "Emails", "Calls"]);
    data.topContacts.forEach(c => rows.push([c.name, c.emailCount, c.callCount]));
    rows.push([]);
    rows.push(["SEQUENCES"]);
    rows.push(["Name", "Steps", "Enrollments"]);
    data.sequenceStats.forEach(s => rows.push([s.name, s.steps, s.enrollments]));
